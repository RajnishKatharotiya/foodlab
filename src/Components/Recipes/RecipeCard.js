...
const RecipeCard = ({ title, img, id, recipe }) => {
    ...
    return (
        <Card className="bg-dark text-white">
            ...
            <Card.ImgOverlay onClick={() => setDetailCard(true)}>
                ...
                <h3 className="recipe_card-price-box">${recipe.price}</h3>
            </Card.ImgOverlay>

            <RecipeDetailCard id={id} show={showDetail} recipe={recipe} onHide={() => setDetailCard(false)} title={title} handleUpdateStorage={handleUpdateStorage} fav={fav} cart={cart} />
        </Card>
    )
}
